...
-- Fungsi untuk memeriksa dan mencetak kesalahan jika folder atau objek tidak ditemukan
local function checkObjectExistence(parent, name)
...
local RemoteInterface = checkObjectExistence(ReplicatedStorage, "RemoteInterface")
if RemoteInterface then
    local Interactions = checkObjectExistence(RemoteInterface, "Interactions")

    if Interactions then
        -- Remote Events
        local shotHitPlayer = checkObjectExistence(Interactions, "shotHitPlayer")
        local shotHitAi = checkObjectExistence(Interactions, "shotHitAi")
        local objectHit = checkObjectExistence(Interactions, "objectHit")
        local hitStructure = checkObjectExistence(Interactions, "hitStructure")
        local shotStructure = checkObjectExistence(Interactions, "shotStructure")
        local chop = checkObjectExistence(Interactions, "chop")
        local buyRebirthPerk = checkObjectExistence(Interactions, "buyRebirthPerk")
        local deleteStructure = checkObjectExistence(Interactions, "deleteStructure")
        local eat = checkObjectExistence(Interactions, "eat")
        local mine = checkObjectExistence(Interactions, "mine")
        local meleeAl = checkObjectExistence(Interactions, "meleeAl")
        local meleePlayer = checkObjectExistence(Interactions, "meleePlayer")
        local rebirth = checkObjectExistence(Interactions, "rebirth")
        local plant = checkObjectExistence(Interactions, "plant")
        local removePath = checkObjectExistence(Interactions, "removePath")
        local itemAlInterraction = checkObjectExistence(Interactions, "itemAlInterraction")
